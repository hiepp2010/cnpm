...
const BasicModal = (props) => {
  ...
  const css =
    "bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 border border-blue-70 ";
  const newData = [
    {
      id: "1",
      name: "Hoàng",
    ...
    <table className="border-collapse border min-w-full bg-white border border-gray-300 mt-4">
      <thead>
        <tr>
          ...
          <th className="border py-2 px-4 border-b">Thao tác</th>
        </tr>
      </thead>
      <tbody>
        {data.map((row, index) =>
            ...
            row.id.includes(selectedId) ? (
              <tr key={index}>
                ...
                <td className="border py-2 px-4 border-b">
                  <BasicModal data={data} setData={setData} id={index} />
                </td>
              </tr>
            ) : (
              ""
